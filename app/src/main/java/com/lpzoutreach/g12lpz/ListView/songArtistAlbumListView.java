...
public class songArtistAlbumListView extends ArrayAdapter<MusicModel> {
    private Context context;
    private List<MusicModel> data;
    private int id;

    public songArtistAlbumListView(@NonNull Context context, int id, List<MusicModel> data ){
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        title.setText(rows.getAlbumName());
        subTitle.setText(rows.getAlbumYearReleased());

        card.setOnClickListener(v -> rows.getEventListenerUtil().myCallBack(rows.getActionEvent(), rows.getAlbumID() + "~" + rows.getAlbumName()));

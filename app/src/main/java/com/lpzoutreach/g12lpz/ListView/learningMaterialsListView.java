...
public class learningMaterialsListView extends ArrayAdapter<BookModel> {

    private Context context;
    private List<BookModel> data;

    public learningMaterialsListView(@NonNull Context context, int resource, @NonNull List<BookModel> objects) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(!rows.getBook_logo().isEmpty()){
            Picasso.get().load(accessUrl.BASE_URL + rows.getBook_logo())
                    .placeholder(context.getDrawable(R.drawable.default_photo))
                    .error(context.getDrawable(R.drawable.default_photo))
                    .into(logo);
        }

        title.setText(rows.getBook_name());
        subtitle.setText(rows.getBook_description());
        Language.setText(rows.getBook_language());
        TotalChapters.setText("Chapter " + String.valueOf(rows.getBook_total_chapters()));
        ...
        card.setOnClickListener(v ->{
            //icon.setImageResource(R.drawable.ic_icon_arrow_right);
            rows.getEventListenerUtil().myCallBack("listview-click", rows.getBookID());
                }
